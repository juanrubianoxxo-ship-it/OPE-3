...
def _get_configured_onedrive_url() -> str:
    """Obtiene el vínculo desde secrets o variable de entorno, si existe."""
    configured = os.getenv(ONEDRIVE_URL_ENV, "").strip()
    try:
        secret_value = str(st.secrets.get(ONEDRIVE_URL_ENV, "")).strip()
    except Exception:
        secret_value = ""
...
def _fetch_onedrive_file(shared_url: str) -> tuple[bytes, str, str]:
    """Descarga el libro y devuelve contenido, fecha remota y URL final."""
    if not shared_url:
        raise ValueError("No se configuró un vínculo de OneDrive.")
    candidates = [_onedrive_api_url(shared_url), shared_url]
    last_error = None
    for candidate in candidates:
        try:
            ...
            if len(response.content) < 1000 or "text/html" in content_type:
                raise ValueError("El vínculo no devolvió un archivo Excel descargable.")
            remote_modified = response.headers.get("last-modified", "")
            return response.content, remote_modified, response.url
        except Exception as exc:
            last_error = exc
    raise RuntimeError(f"No fue posible descargar el archivo de OneDrive: {last_error}")
